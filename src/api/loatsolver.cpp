...
LoatResult LoatSolver::check()
{
    ...
    if (res != SmtResult::Unknown) {
        std::cout << "[LoAT] Trivial Analysis result: "
                  << (res == SmtResult::Sat ? "SAT" : res == SmtResult::Unsat ? "UNSAT"
                                                                              : "UNKNOWN")
                  << std::endl;
        std::cout << "[LoAT] Solved by Trivial Analysis" << std::endl;
        // If the user wanted a proof, save it
        if (Config::Analysis::model) {
            if (res == SmtResult::Sat) {
                if (Config::Analysis::log) {
                    std::cout << "[LoAT] Transforming ITS model" << std::endl;
                }
                its_model = trivial_solver.get_model();
            } else {
                if (Config::Analysis::log) {
                    std::cout << "[LoAT] Transforming ITS cex" << std::endl;
                }
                its_cex = trivial_solver.get_cex();
            }
        }
    } else {
    ...
    else if (its_cex.has_value())
    {
        ...
        m_its_cex = LoatCex(preprocessor->transform_cex(std::make_shared<ITSSafetyCex>(*its_cex)));
    }
