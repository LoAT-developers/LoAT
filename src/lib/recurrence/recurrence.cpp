...
bool Recurrence::solve_purrs(const Arith::Var lhs, const Arith::Expr rhs) {
    auto [updated, map] {arith::toPurrs(closed_form_n_minus_one.get<Arith>()(rhs), this->n)};
    ...
    result.prefix = std::max(result.prefix, prefix);
    closed_form_n_minus_one.put<Arith>(lhs, arith::fromPurrs(closed_form.substitute(Purrs::Recurrence::n, map.left.at(this->n)-1), map));
