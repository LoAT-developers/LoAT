...
using LhsPtr = cpp::not_null<std::shared_ptr<const Lhs>>;
...
class Lhs {
    ...
    struct CacheEqual {
        bool operator()(const std::tuple<std::string, std::vector<Var>> &args1, const std::tuple<std::string, std::vector<Var>> &args2) const noexcept;
    };
    struct CacheHash {
        size_t operator()(const std::tuple<std::string, std::vector<Var>> &args) const noexcept;
    };
    static ConsHash<Lhs, Lhs, CacheHash, CacheEqual, std::string, std::vector<Var>> cache;

public:

    Lhs(const std::string &pred, const std::vector<Var> &args);
    ~Lhs();
    static LhsPtr mk(const std::string &pred, const std::vector<Var> &args);
    ...
    friend std::ostream& operator<<(std::ostream &s, const LhsPtr f);
    LhsPtr subs(const Subs &subs) const;
    LhsPtr rename_vars(const Renaming &) const;
...
private:
    ...
    using Args = std::tuple<std::optional<LhsPtr>, Bools::Expr, std::optional<FunAppPtr>>;
    ...
    static ConsHash<Clause, Clause, CacheHash, CacheEqual, std::optional<LhsPtr>, Bools::Expr, std::optional<FunAppPtr>> cache;

public:

    Clause(const std::optional<LhsPtr> premise, const Bools::Expr constraint, const std::optional<FunAppPtr> conclusion);
    ~Clause();

    static ClausePtr mk(const std::optional<LhsPtr> premise, const Bools::Expr constraint, const std::optional<FunAppPtr> conclusion);

    bool is_fact() const;
    bool is_query() const;
    bool is_left_linear() const;
    std::optional<LhsPtr> get_premise() const;
