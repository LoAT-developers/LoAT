...
Bools::Expr real_mbp(const Bools::Expr &t, const ModelPtr &model, const ArithVarPtr& x, Subs& subs) {
    auto bounds {t->getBounds(x)};
    const auto it {std::ranges::find_if(bounds, [](const auto &b) {
        return b.kind == BoundKind::Equality;
    })};
    if (it != bounds.end()) {
        subs = subs.compose(Subs::build(x->var(), arrays::update(x, it->bound)));
        return t;
    }
    if (const auto closest {mbp::closest_lower_bound(bounds, model, x)}) {
        subs = subs.compose(Subs::build(x->var(), arrays::update(x, *closest)));
        return t;
    }
    return t->toMinusInfinity(x);
}

Bools::Expr real_mbp(const Bools::Expr& t, const ModelPtr& model, const Cell& x, Subs& subs) {
    return theory::apply(
        x,
        [&](const Bools::Var& x) {
        ...
        [&](const ArithVarPtr& x) {
            const auto res{real_mbp(t, model, x, subs)};
            assert(res != bot());
            return res;
        });
}
