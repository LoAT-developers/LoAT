...
Rational ArithExpr::max_coeff() const {
    return apply<Rational>(
        [&](const ArithConstPtr c) {
            return 0;
        },
        ...
        [&](const ArithAddPtr a) {
            const auto &args {a->getArgs()};
            auto it {args.begin()};
            auto res {(*it)->max_coeff()};
            for (++it; it != args.end(); ++it) {
                res = mp::max(res, (*it)->max_coeff());
            }
            return res;
        },
        [&](const ArithMultPtr m) -> Rational {
            for (const auto &arg: m->getArgs()) {
                if (const auto c {arg->isRational()}) {
                    return mp::abs(***c);
                }
            }
            return 1;
        ...
        [&](const ArithModPtr m) {
            return mp::max(m->getLhs()->max_coeff(), m->getRhs()->max_coeff());
        },
        [&](const ArithExpPtr e) {
            return 1;
        });
...
Rational ArithExpr::coeff_sum() const {
    return apply<Rational>(
        [&](const ArithConstPtr c) {
            return 0;
        },
        ...
        [&](const ArithModPtr m) {
            return m->getLhs()->coeff_sum() + m->getRhs()->coeff_sum();
        },
        [&](const ArithExpPtr e) {
            return 1;
        });
