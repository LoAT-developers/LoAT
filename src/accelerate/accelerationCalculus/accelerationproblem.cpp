...
AccelerationProblem::AccelerationProblem(
    ...
    SmtFactory::SmtConfig config(Smt<IntTheory, BoolTheory>::chooseLogic<LitSet, Subs>({guard->lits()}, subs));
    config.incremental = true;
    config.produce_models = true;
    this->solver = SmtFactory::solver<IntTheory, BoolTheory>(config, its);
