...
AccelerationTechnique::Accelerator::Accelerator(const BoolExpr &formula, const Proof &proof, bool exact, bool witnessesNonterm):
    formula(formula), proof(proof), exact(exact), witnessesNonterm(witnessesNonterm) {}

AccelerationTechnique::AccelerationTechnique(
...
option<Recurrence::Result> AccelerationTechnique::getClosed() const {
    return closed;
}
